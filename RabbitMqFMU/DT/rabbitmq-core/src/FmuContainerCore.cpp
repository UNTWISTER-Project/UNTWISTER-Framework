...
#include "./../../rabbitmq-fmu/src/Iso8601TimeParser.h"
#include <iostream>
#include <iomanip>
#include <chrono>
#include <cmath>
...
FmuContainerCore::FmuContainerCore(std::chrono::milliseconds maxAge, std::map<ScalarVariableId, int> lookAhead)
        : maxAge(maxAge), lookahead(lookAhead), startOffsetTime(std::chrono::milliseconds(0)), verbose(true){
...
void FmuContainerCore::add(ScalarVariableId id, TimedScalarBasicValue value) {
    ...
    /* std::size_t size = this->incomingUnprocessed[id].size();
...
std::chrono::milliseconds FmuContainerCore::messageTimeToSim(date::sys_time<std::chrono::milliseconds> messageTime) {
    //cout << "messageTime: " << Iso8601::convertTimestampToString(messageTime) << " startoffset: " << Iso8601::convertTimestampToString(this->startOffsetTime)<< endl;
...
void FmuContainerCore::processIncoming() {
    for (auto itr = this->incomingUnprocessed.begin(); itr != this->incomingUnprocessed.end();) {
        //cout<<"IDIncomingProcessed: " << itr->first << endl;
        itr++;
    }
    //sort
    for (auto &pair: this->incomingUnprocessed) {
        ...
        //move
        //std::cout << "Size of incomingLookahead[" << id << "] is now: "
          //<< this->incomingLookahead[id].size() << std::endl;
    ...
    if (!this->incomingUnprocessed.empty()) {
        ...
        for (auto itr = this->incomingUnprocessed.begin(); itr != this->incomingUnprocessed.end();) {
            ...
            //if(this->incomingUnprocessed == NULL) 
                //cout<<"IDIncomingProcessed: "<<endl;
            for (auto itr = this->incomingUnprocessed.begin(); itr != this->incomingUnprocessed.end();) {
                //cout<<"IDIncomingProcessed: " << itr->first << endl;
                itr++;
            }
...
bool FmuContainerCore::initialize() {
    ...
    if (initialTimePair.first) {
        ...
        //cout<<"START OFFSET TIME: " << Iso8601::convertTimestampToString(this->startOffsetTime) << endl;
...
//read until time
    ...
    auto predicate = [time, this](FmuContainerCore::TimedScalarBasicValue &value) {
        //cout << "messageTime: " << Iso8601::convertTimestampToString(value.first) << " startoffset: " << Iso8601::convertTimestampToString(this->startOffsetTime)<< endl;
        //cout<<"Timestamp: "<<value.first<<" startOffset: "<<this->startOffsetTime<<endl;
        //cout<<"predicate: "<< messageTimeToSim(value.first).count() << " time: "<< time<<endl;
        return messageTimeToSim(value.first).count() <= time;
    };
    //cout<<"TRE"<<endl;
    processLookahead(predicate);
...
bool FmuContainerCore::check(double time) {
    ...
    for (auto &lookaheadPair: this->lookahead) {
        ...
        auto valueTime = this->currentData.at(id).first;
        //auto stopmode = this->currentData.at(id).second;
        ...
        //this->printStartOffsetTime(this->startOffsetTime);

        /* double valueTimePlusMaxAge = static_cast<double>((messageTimeToSim(valueTime) + this->maxAge).count());

        // Round or truncate bo and time to 3 decimal digits
        double roundedvalueTimePlusMaxAge = std::round(valueTimePlusMaxAge * 1000000.0) / 1000000.0; 
        double roundedTime = std::round(time * 1000000.0) / 1000000.0;


        //std::cout<<"CURRENT TIME CHECK: " << std::chrono::system_clock::now() << endl;
        //printf("(messageTimeToSim(valueTime) + this->maxAge).count(): %f time: %f\n", roundedvalueTimePlusMaxAge, roundedTime);

        if (roundedvalueTimePlusMaxAge < roundedTime) {
            return false;
        } */
        ...
        this->lastTimestamp = valueTime;
        /*  if(id == 100) {
            this->lastCommand = this->currentData.at(id).second.d.d;
        } else if (id == 107) {
            this->stopmode = this->currentData.at(id).second.b.b;
        } */
        //std::cout << "stopmode = " << std::boolalpha << stopmode.b.b << std::endl;
        //cout<<"lastTimeStamp: "<<this->lastTimestamp<<endl;
        //this->lastCommand = this->currentData.at(id).third.d.d;
    }
    return true;
}

void FmuContainerCore::printStartOffsetTime(const date::sys_time<std::chrono::milliseconds>& startOffsetTime) {
    // Convert startOffsetTime to a time_point
    auto time_point = date::floor<std::chrono::milliseconds>(startOffsetTime);

    // Convert time_point to a time_t
    auto time_t = std::chrono::system_clock::to_time_t(time_point);

    // Convert time_t to a std::tm structure
    std::tm* tm_ptr = std::localtime(&time_t); // Use std::gmtime for UTC time

    // Extract milliseconds from time_point
    auto milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(time_point.time_since_epoch()) % std::chrono::seconds(1);

    // Format std::tm structure as a string including milliseconds
    std::ostringstream oss;
    oss << std::put_time(tm_ptr, "%Y-%m-%d %H:%M:%S") << "." << std::setfill('0') << std::setw(3) << milliseconds.count();

    // Print the formatted date string
    std::cout << "Start Offset Time: " << oss.str() << std::endl;
}
