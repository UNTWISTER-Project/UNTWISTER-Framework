...
void FmuContainer::consumerThreadFunc(void) {
    ...
    while (!consumerThreadStop) {
        //int queue_size = this->coreIncomingSize();  // ACCESSO A SEZIONE CRITICA
        //FmuContainer_LOG(fmi2OK, "logOk", "queue size '%d'", queue_size);

        if(true)
        {
            std::unique_lock<std::mutex> lock(this->core->m);
            cv.wait(lock, [this] {return this->ready;}); // waits until "ready" becomes true
            this->ready = false;
            lock.unlock();
            if (this->rabbitMqHandlerConsume->consume(json)) {
                        ...
                        if (MessageParser::parse(&this->nameToValueReference, json.c_str(), &result)) {
                            ...
                            date::sys_time<std::chrono::milliseconds> timestampPlusSimTimeMilliseconds;
                            // add simstep ms
                            if(this->firstTimestamp) {
                                    timestampPlusSimTimeMilliseconds = this->core->lastTimestamp;
                                    this->firstTimestamp = false;
                                }
                            else  
                                timestampPlusSimTimeMilliseconds = this->core->lastTimestamp + std::chrono::milliseconds(this->simstep);
        ...
        /* else
        {
            ...
            std::chrono::milliseconds timespan(2); // or whatever
    ...
    try {
        ...
        while (((std::chrono::duration<double>) (std::chrono::system_clock::now() - start)).count() <
            ...
            if (this->rabbitMqHandler->consume(json)) {
                ...
                std::string currentTime = Iso8601::getCurrentTimestamp();
            ...
            /* else {
                ...
                bool stopMode = false;
                std::string message = R"({"time": ")" + currentTimestamp + R"(, "lead_acceleration": 0})";
