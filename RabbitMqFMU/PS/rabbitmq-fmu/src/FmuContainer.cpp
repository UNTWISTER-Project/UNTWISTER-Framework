...
FmuContainer::~FmuContainer() {
    ...
    if (this->rabbitMqHandlerConsume) {
        ...
        cout<<"I'm closing the connection"<<endl;
        ...
        if(true)
        {
            if (this->rabbitMqHandlerConsume->consume(json)) {
                        ...
                        if (MessageParser::parse(&this->nameToValueReference, json.c_str(), &result)) {
                            ...
                            //std::printf("logOk, Got data %s, %s, %lld", startTimeStamp.str().c_str(), json.c_str(), std::chrono::high_resolution_clock::now());

                            std::unique_lock<std::mutex> lock(this->core->m);
                                                        // add simstep ms
                            auto timestampPlusSimTimeMilliseconds = this->core->lastTimestamp + std::chrono::milliseconds(this->simstep);
                            result.time = timestampPlusSimTimeMilliseconds;
                            for (auto &pair: result.integerValues) {
                                ...
                                /* } */
                                FmuContainer_LOG(fmi2OK, "logWarn", "pair.first'%d'", pair.first);
                            ...
                            for (auto &pair: result.booleanValues) {
                                cout<<"stop_mode: "<< pair.second<<endl;
                                this->core->add(pair.first, std::make_pair(result.time, pair.second));
                                FmuContainer_LOG(fmi2OK, "logWarn", "pair.first'%d'", pair.first);
                            }
                            lock.unlock();
                            ...
                            cv.notify_one();
                            //std::cout<<"exiting from consumer thread: " << std::chrono::system_clock::now() << endl;
                            FmuContainer_LOG(fmi2OK, "logOk", "exiting from consumer thread%s", "");
                        } else {
...
bool FmuContainer::initialize() {
    ...
    auto intConfigs = {std::make_pair(RABBITMQ_FMU_PORT, "port"),
                       ...
                       std::make_pair(RABBITMQ_FMU_VHOST, "simstep")};
    ...
    this->precisionDecimalPlaces = this->currentData.integerValues[RABBITMQ_FMU_PRECISION];
    ...
    this->simstep = this->currentData.integerValues[RABBITMQ_FMU_SIMSTEP];

    cout<<"simstep: "<< this->simstep<<endl;

    if (this->precisionDecimalPlaces < 1) {
        FmuContainer_LOG(fmi2Fatal, "logAll",
                         "Precision must be a positive number %d",
                         this->precisionDecimalPlaces);
        return false;
    }

    this->precision = std::pow(10, this->precisionDecimalPlaces);
    ...
    FmuContainer_LOG(fmi2OK, "logAll",
                     ...
                     this->communicationTimeout, this->precision, this->precisionDecimalPlaces, useSSL);
    ...
    FmuContainer_LOG(fmi2Warning, "logWarn", "I m sending the init message %s", "");
    ...
    FmuContainer_LOG(fmi2Warning, "logWarn", "core initializated %s", "");
    ...
    std::cout << "end initialize: " << std::chrono::system_clock::now() << endl;
    return true;
}
...
#endif
                //data received
                cout<<"json: "<<json.c_str()<<endl;
                DataPoint result;
                if (MessageParser::parse(&this->nameToValueReference, json.c_str(), &result)) {

                    //std::stringstream startTimeStamp;
                    //startTimeStamp << result.time;
                    //this->lastTimestamp = Iso8601::convertTimestampToString(result.time);
...
//                        cout << *this->core;
                        //std::cout<<"exiting from the first consume thread: " << std::chrono::system_clock::now() << endl;
            ...
            } // TOGLIERE COMMENTO SE NON VOGLIAMO ASPETTARE IL MESSAGGIO 
            else {
                FmuContainer_LOG(fmi2Warning, "logWarn", "I m sending the first publish message %s", "");
                std::string currentTimestamp = Iso8601::getCurrentTimestamp();
                bool stopMode = false;
                std::string message = R"({"time": ")" + currentTimestamp + R"(", "stop_mode": )" +
                    (stopMode ? "true" : "false") +R"(, "lead_acceleration": -1)" +R"(, "ego_acceleration": -1})";
                this->rabbitMqHandler->publish(this->rabbitMqHandler->bindingKey,
                    message,this->rabbitMqHandler->channelPub, this->rabbitMqHandler->rbmqExchange);
                cout<<"First message published: "<<currentTimestamp<<endl;
            }   
        }
...
bool FmuContainer::step(fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize) {
    ...
    //cout<<this->core->stopmode<<endl;

    // Compute simulation time with rounding to 3rd least significant digit (ms precision)
    auto simulationTime = std::round((secondsToMs(currentCommunicationPoint + communicationStepSize) - this->simstep) * 1000.0) / 1000.0;
    //cout <<"SIMULATION TIME: "<< simulationTime << endl;

    FmuContainer_LOG(fmi2OK, "logAll", "************ Enter FmuContainer::step ***************%s", "");
    //std::cout<<"enter step " << std::chrono::system_clock::now() << endl;
    ...
    //std::cout<<"before process 1: " << std::chrono::system_clock::now() << endl

    //cout<<"before process 1"<<endl;
    if (this->core->process(simulationTime)) {
        //std::cout<<"after process 1 with data: " << std::chrono::system_clock::now() << endl;
        ...
        if(!this->core->hasUnprocessed()) {
            ...
            newMessage.time = timestampPlusSimTimeMilliseconds;
            newMessage.booleanValues[107] = false;
            newMessage.doubleValues[100] = this->core->lastLeadAcceleration;
            newMessage.doubleValues[111] = this->core->lastEgoAcceleration;
            this->addToCore(newMessage);
        }

        if (this->core->process(simulationTime)) {
            //std::cout<<"after process 2 with data: " << std::chrono::system_clock::now() << endl;
        ...
        //std::cout<<"after process 2 without data: " << std::chrono::system_clock::now() << endl;
        return false;
    }
    
    //std::cout<<"after process 1 without data: " << std::chrono::system_clock::now() << endl;
    ...
    try {
        ...
        //while (((std::chrono::duration<double>) (std::chrono::system_clock::now() - start)).count() <
            ...
            FmuContainer_LOG(fmi2Warning, "logWarn",
                             "Single thread: I m waiting data from rabbitMq %s", "");
            if (this->rabbitMqHandler->consume(json)) {
           	 FmuContainer_LOG(fmi2Warning, "logWarn",
                             "The message arrived %s", json.c_str());
                //data received
                LOG_TIME(2);
                DataPoint result;
                if (MessageParser::parse(&this->nameToValueReference, json.c_str(), &result)) {
        ...
        if(it->second.input){
            if(it->second.type == ModelDescriptionParser::ScalarVariable::SvType::Real){
                ...
                if(this->currentData.doubleValues[it->second.valueReference] != this->previousInputs.doubleValues[it->second.valueReference] || this->howtosend){
                    ...
                    val << std::fixed << std::setprecision(this->precisionDecimalPlaces) << this->currentData.doubleValues[it->second.valueReference];
